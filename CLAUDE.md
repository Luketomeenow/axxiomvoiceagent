...
- **Stale-call sweeper**: `sweepStaleCalls()` runs at the top of every tick and closes any `call` stuck in `queued/ringing/in-progress` for >15 min (a missed end-of-call webhook would otherwise pin a concurrency slot forever and silently halt dialing — this was a real incident). Sets `ended_reason='stale-timeout'`.
