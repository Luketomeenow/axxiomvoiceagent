 ...
 *  - Vapi (Claude): your production agent — placed over the phone via the Test card above.
 ...
 * The ElevenLabs API key stays server-side (the backend issues a signed URL).
 */
export function AgentSwitcher() {
  ...
  async function startEl() {
    ...
    try {
      ...
      await conversation.startSession({ signedUrl: r.signedUrl });
    } catch (e) {
      setError(String(e));
    } finally {
      setBusy(false);
  ...
  return (
    <div className="card card-pad">
      ...
      <p className="mt-0.5 text-xs text-slate-400">Pick which agent to try. Switching here is for evaluation only — your live campaign still runs on Vapi.</p>
      ...
      <div className="mt-4">
        {provider === "vapi" ? (
          <p className="text-sm text-slate-300">
            Vapi is your production agent. Place a real phone call from the <span className="font-semibold">Test the agent</span> card above.
          </p>
        ) : (
